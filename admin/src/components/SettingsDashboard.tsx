...
export const SettingsDashboard: React.FC = () => {
	...
	const fetchSettings = async () => {
		try {
			...
			setStatusMsg({ type: 'error', text: error.message || 'Error fetching settings.' });
		} finally {
			setLoading(false);
		}
	};
	...
	const handleSave = async (e: React.FormEvent) => {
		...
		try {
			...
			setStatusMsg({ type: 'error', text: error.message || 'Error saving settings.' });
		} finally {
			setSaving(false);
		}
	};
	...
	const handleSyncModels = async () => {
		...
		try {
			...
			setTimeout(async () => {
				await fetchModels();
				setStatusMsg({ type: 'success', text: 'Model catalog synchronized and updated.' });
				setSyncing(false);
			}, 3000);
		} catch (error: any) {
			setStatusMsg({ type: 'error', text: error.message || 'Error syncing models.' });
			setSyncing(false);
		}
	};
		...
		<div className="iris-settings">
			<div className="iris-settings__header">
				<div className="iris-settings__branding">
					<svg className="iris-settings__logo" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM13 17H11V15H13V17ZM13 13H11V7H13V13Z" fill="currentColor"/>
					</svg>
					<div>
						<h1 className="iris-settings__title">Iris Assistant</h1>
						<p className="iris-settings__subtitle">Configure your WordPress AI Copilot, model parameters, and preferences.</p>
					</div>
				</div>
				<button
					type="button"
			...
			{statusMsg && (
				<div className={`iris-settings__alert iris-settings__alert--${statusMsg.type}`}>
					<span className="iris-settings__alert-text">{statusMsg.text}</span>
					<button className="iris-settings__alert-close" onClick={() => setStatusMsg(null)}>
						&times;
					</button>
				</div>
			)}
			...
			<form className="iris-settings__form" onSubmit={handleSave}>
					...
					<div className="iris-settings__form-group">
						...
						<div className="iris-settings__input-wrapper">
							<input
								...
								placeholder={hasApiKey ? '••••••••••••••••••••••••••••••••' : 'sk-or-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'}
								value={apiKey}
				...
				<div className="iris-settings__section">
					...
					<div className="iris-settings__form-group">
							...
							>
								{activeModelObj ? (
									<div className="iris-settings__selected-model">
										<span className="iris-settings__selected-name">{activeModelObj.name}</span>
										<span className="iris-settings__selected-meta">
											{activeModelObj.context_length.toLocaleString()} ctx &bull; {getModelPricingStr(activeModelObj)}
										</span>
									</div>
								) : (
								...
								<svg className="iris-settings__dropdown-arrow" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
								...
								<div className="iris-settings__model-menu">
									<div className="iris-settings__model-menu-header">
										<input
											...
											autoFocus
										/>
										<label className="iris-settings__free-filter" onClick={(e) => e.stopPropagation()}>
											<input
												type="checkbox"
												checked={freeOnly}
											...
											filteredModels.map((model) => (
												...
												>
													...
													<div className="iris-settings__option-meta">
														{model.id} &bull; {(model.context_length / 1000).toFixed(0)}k context &bull; {getModelPricingStr(model)}
													</div>
												</div>
											))
										) : (
											<div className="iris-settings__model-no-results">No models found matching criteria.</div>
										)}
									</div>
								</div>
							)}
						...
						<p className="iris-settings__help-text">
							Select the LLM that Iris will query. Models marked as Free do not incur billing charges on your OpenRouter account.
						</p>
					</div>
				</div>
				...
				<div className="iris-settings__section">
					<h2 className="iris-settings__section-title">Assistant Instructions</h2>
					...
					<div className="iris-settings__form-group">
						...
						/>
						<p className="iris-settings__help-text">
							Custom instructions prepended to the message context. Guides the tone, boundaries, and responsiveness of the model.
						</p>
					</div>
					...
					<div className="iris-settings__form-group iris-settings__form-group--checkbox">
						<label className="iris-settings__checkbox-label">
							...
							<span className="iris-settings__checkbox-text">
								Share site telemetry to help Iris give better answers (WordPress version, active plugins, and active theme).
							</span>
						</label>
					</div>
				</div>
				...
				<div className="iris-settings__section">
					...
					<div className="iris-settings__grid">
						...
						<div className="iris-settings__form-group">
							...
							/>
							<p className="iris-settings__help-text">Maximum length of the assistant response.</p>
						</div>
					</div>
				</div>

				<div className="iris-settings__footer">
					<button type="submit" className="iris-settings__save-btn" disabled={saving}>
						{saving ? (
							<>
